...
import com.datasift.dropwizard.scala.jdbi.tweak.{OptionContainerFactory, OptionArgumentFactory, BigDecimalArgumentFactory, IterableContainerFactory}
...
object JDBI {
  ...
  def apply(env: Environment, conf: DataSourceFactory, name: String): DBI = {
    ...
    dbi.registerArgumentFactory(new BigDecimalArgumentFactory(conf.getDriverClass))
